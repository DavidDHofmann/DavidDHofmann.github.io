...
image: "img/backgrounds/snareddogs.jpg"
output:
  blogdown::html_page:
    highlight: tango
    toc: false
...
knitr::include_graphics("img/SnaredDog1.jpg", error = F)
```
...
knitr::include_graphics("img/SnaredDog2.jpg", error = F)
```
...
knitr::include_graphics("img/DogSnares.jpg", error = F)
```
...
```{r, echo=F, messages=F, out.width="50%", fig.cap="Right after we removed the last snare, the dogs started to move and prepared for a hunt. The front dog is the dominant male, who now carries a GPS satellite collar that allows us to monitor the pack’s movements remotely."}
knitr::include_graphics("img/Dijo.jpg", error = F)
```
...
```{r, echo=F, messages=F, out.width="100%", fig.cap="Movement route of the pack two weeks prior to the snare removal. As you can see from their trajectory, the dogs passed by the village of Mababe, a village known for its illegal trapping and hunting activities."}
knitr::include_graphics("img/SnaredDogsRoute.png", error = F)
```
