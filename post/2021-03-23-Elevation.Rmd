...
```{r focal, message = F, warning = F, echo = F, fig.width = 2, fig.height = 2, fig.cap = "\\label{fig:focal} Illustration how the focal function works. The function uses a moving window within which a metric can be applied. That is, the new value of the focal cell will be determined by the values within the moving window and the specified function."}
...
# Identify neighbors
adj <- adjacent(r, center, directions = 8)[, 2]

# Highlight focal cell and its neighbors
par(mar = c(0, 0, 0, 0))
plot(rp)
plot(rp[adj, ], col = "red", add = T)
plot(rp[center, ], col = "red", border = "yellow", add = T, lwd = 2)
legend("bottom", c("Focal Cell", "Moving Window"), col = c("yellow", "red"), pch = c(0, 15))

```

We can thus use the `focal` function to calculate the ruggedness for our study
area.
