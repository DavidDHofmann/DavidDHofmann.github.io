...
image: "img/post_okavangodelta.jpg"
output:
  blogdown::html_page:
    highlight: tango
    toc: true
...
knitr::include_graphics("/img/gallery_floodmapping/DeltaFromSpace.png", error = F)
```
...
knitr::include_graphics("/img/gallery_floodmapping/DeltaTwoSeasons.png", error = F)
```
...
files. We'll pass the filepaths into the `modis_load()` (which is a wrapper for
the `terra::rast()` function) function to load the generated rasters into our
R-session.
...
knitr::include_graphics("/img/gallery_floodmapping/FloodmappingPolygons.png", error = F)
```
