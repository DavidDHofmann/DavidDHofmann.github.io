...
date: 2021-04-06T18:00:00-05:00
categories: ["R"]
tags: ["R", "deep learning", "cameratrap"]
...
# Introduction
Wildlife cameras have become increasingly popular to monitor and study animal
populations. They are non-invasive and, once set up, can be left in the field
for months. Thereby, the collection of images can easily be automated and with
only little effort exciting insights into wildlife can be gained. On the
downside, wildlife cameras often produce thousands of images that need to be
screened by trained biologists. This can be very time-consuiming and cumbersum,
particularly if there are many false positives, i.e. images on which there are
no animals.
...
With the uprise of deep learning techniques, such issues can now be tackled
using computers. For cameratrapping data in particular, Microsoft has recently
released a neural network called the "MegaDetector". The MegaDetector is a
powerful neural network that has been trained to distinguish the classes
"humans", "vehicles", and "animals" on any image. For this, it has been fed with
millions of cameratrap images with known detections and known absences. Albeit
the detector only detects these three calsses, it does so with great
reliability. Consequently, the MegaDetector may serve as a first filter to clean
and remove some of the images on which no animals can be seen. In this
blog-post, I want to outline a simple workflow to show you how you can apply the
MegaDetector on your own set of images.

# Installation
Probably the most difficult part of using the MegaDetector is to get everything
installed, especially if you are not familiar with GitHub, Python, pip, etc. I
will therefore try to be very thorough and explain each step in as much detail
as I can. Nevertheless, I will have to refer you to other sources in some cases.
Much of this blog post is inspired from [Microsoft's Github
Instructions](https://github.com/microsoft/CameraTraps/blob/master/megadetector.md#using-the-models)
on how to use the MegaDetector.
...
environments containing packages of a desired version. This drastically reduces
the need to manually fiddle with packages to ensure correct versions. Because
the installation of conda differs depending on your operating system, I will ask
you to follow the installation instructions here:

* [Windows](https://conda.io/projects/conda/en/latest/user-guide/install/windows.html)
* [MacOS](https://conda.io/projects/conda/en/latest/user-guide/install/macos.html)
* [Linux](https://conda.io/projects/conda/en/latest/user-guide/install/linux.html#)

Once you have conda installed, open up a new terminal. You should now see an
indicator telling you that conda booted into the base environment (Figure 2(a)
further below).

As stated above, it is very easy to create new python environments using. In
fact, we are going to create a new environment called *cameratrapping* that uses
python version 3.6 right now. To do so, we run the following command:
...
Note that we specifically ask for python version 3.6. This is extremely
...
After activation, you're terminal should look somewhat similar to Figure 2(b)
and indicate that conda is working in the *cameratrapping* environment.

```{r, echo=F, messages=F, out.width="75%", fig.cap='Conda always tells you in which environment you are currently working. To switch environments, use "conda activate environment-name".'}
knitr::include_graphics("/img/gallery_megadetector/figure3.png", error = F)
```

Using `activate environment-name` you can always go back and forth between
...
For now, we only care about relatively certain detections, so I'll subset the
data accordingly.
...
Most often, you will want to visualize the detections. For this, we'll need to
load the respective image into R, draw a bounding box around the detected object
and plot everything. We can write a function that allows us to automate these
tasks:

```{r, warning=F, message=F, out.width="100%", fig.cap="Some example detections on cameratrap images."}
