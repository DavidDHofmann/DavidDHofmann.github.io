...
rely on point estimates. However, in reality there is often considerable
uncertainty such that point estimates are always very accurate. Consequently, we
are sometimes interested in the **sensitivity of our predictions** with respect
to changes in estimated parameters. This is exactly what we can examine using
sensitivity analyses. That is, we predict or simulate the outcome of interest
using different model parameters and check how varying parameters influences our
results.

Coding a sensitivity analysis can be a bit of a challenge because it involves
rigorous bookkeeping of employed parameters and corresponding model outputs. It
is therefore often worthwile to invest a bit of time and effort to create
...
sensitivity analysis in R. In short, the approach consists of **two parts**.
First, writing out a **generalized function** that allows us to predict the
outcome of interest for a given set of parameters. Second, to **apply the
function repeatedly** across all combinations of parameters that we want to test
for. For this, I rely on the [tidyverse](https://www.tidyverse.org/) package. In
particular, I will make use of 'tibbles', which are tidyverse's alternative to
dataframes. But enough blabbing, let's get started.

# Model
For the purpose of this blog post, we will look at a simple [matrix population
model](https://en.wikipedia.org/wiki/Matrix_population_models). The same
workflow can of course be applied to most other modeling frameworks too.
...
or simulate the initial population into the future using the following equation:
...
(\(S_j\)) depends on the temperature in the environment and that fecundity of
adults (\(F_a\)) depends on the size of the population (i.e. adult survival is
density dependent). Juvenile fecundity, on the other hand, is set to 0, and
adult survival is set to a probability of 0.9. Hence, the transition matrix will
look as follows:
...
Because we assume that \(F_a\) and \(S_j\) are not constant but depend on the
...
functions that make use of a linear-regression model and a logit-link to
...
As you can see, I have not entered specific values for alpha, beta_1, and beta_2
in the two funtions. This will allow us to later manipulate the functional forms
of \(S_j\) and \(F_a\) to see how they influence our model predictions. Using
the functions it is now straight forward to plot the reaction curves for a given
set of parameters and temperatures/populations.
...
highest at around 20 degrees Celsius. Adult fecundity (\(F_a\)), on the other
hand, is highest when the population size is at its minimum. Thus, there is
negative density dependence, meaning that fecundity decreases as the population
size increases. Overall, the transition matrix \(A\) will vary depending on the
temperature, the population size, and the parameters that define the functional
forms of \(S_j\) and \(F_a\). Instead of manually defining \(A\) for each
desired combination of these variables, let's write a function `createMat()`
that automatically generates the transition matrix \(A\) for a given set of
parameters and a given temperature and population size. As mentioned above, we
will later manipulate the parameters that describe the functional forms of
\(S_j\) and \(F_j\). Hence, the `createMat()` function needs to take those as
arguements.
...
This is a quite powerful function! We can actually use it to compare the
transition matrices resulting under different assumptions. For instance, let's
see how the transition matrix changes when we increase the population size.
...
# Simulation Function
In order to be able to quickly repeat our simulation multiple times and for
different parameters, we want to wrap our population projection into a single
function called `simulation()`. Given an initial population, a vector of
temperatures, and a set of model parameters, the `simulation()` function will
project population dynamics into the future.
...
Now we are ready for our sensitivity analysis and can project our population
into the future using different parameters. This is where the power of tibbles
comes into play. We can actually use the function `grid_expand()` to span a
design matrix that comprises all combinations of parameters that we want to test
for. Moreover, we can specify how many replicates of each combination we wish to
run. While we could vary all six parameters describing the functional forms of
\(S_j\) and \(F_a\), (i.e. \(\alpha_{temp}, \beta_{1, temp}, \beta_{2, temp},
\alpha_{popsize}, \beta_{1, popsize}, \beta_{2, popsize}\)), this will result in
too many different outputs. Hence, we are only going to vary
\(\beta_{temperature_1}\) and \(\beta_{popsize, 1}\) and we will repeat each
simulation 100 times (i.e. 100 replicates). Note that the `grid_expand()`
automatically produces all unique combinations of parameters and replicates.
...
We could now use a "for-loop" to go through each row and apply our
`simulation()` function. However, I prefer to use the `lapply()` function, which
directly returns a list comprising the results of each simulation. This allows
us to conveniently store the resulting output directly in a new column the
"design" tibble, such that we readily know which simulation belongs to which set
of parameters.
...
of the different simulations and their parameters. Moreover, we can subset and
access simulations as we wish.
